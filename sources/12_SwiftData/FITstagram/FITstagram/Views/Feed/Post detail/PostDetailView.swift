...
final class PostDetailViewModel {
    ...
    let postID: Post.ID
    let jsonAPI: JSONAPIServicing
    
    init(postID: Post.ID, jsonAPI: JSONAPIServicing) {
        self.postID = postID
...
//            (post, comments) = try await (fetchPostDetail(), fetchComments())
    ...
    private func fetchPostDetail() async throws -> Post {
        try await jsonAPI.fetchPostDetail(postID: postID)
//        let request = URLRequest(url: .post(id: postID))
//        let (data, _) = try await URLSession.shared.data(for: request)
//        return try JSONDecoder().decode(Post.self, from: data)
    }
...
#Preview {
    PostDetailView(viewModel: .init(postID: "1", jsonAPI: MockJSONAPI()))
}

final class MockJSONAPI: JSONAPIServicing {
    func fetchPostDetail(postID: Post.ID) async throws -> Post {
        .init(
            id: postID,
            author: .userMock1,
...
//#Preview("Red") {
//    PostDetailView(viewModel: .init(postID: "1", jsonAPI: JSONAPIService()))
